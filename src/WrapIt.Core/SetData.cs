...
namespace WrapIt
{
    internal class SetData : InterfaceData
    {
        public TypeData ElementType { get; }

        public SetData(Type type, TypeName className, TypeName interfaceName, TypeData elementType)
        ...
        public override string GetCodeToConvertToActualType(string parameterName) => ClassName != InterfaceName ? $"{parameterName}?.ToSet()" : base.GetCodeToConvertToActualType(parameterName);
